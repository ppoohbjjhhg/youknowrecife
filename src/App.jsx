    ...
    <div className="ykr min-h-screen w-full overflow-x-hidden">
      ...
      <footer className="border-t border-[var(--yn-border)] py-10">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <Logo size={24} />
          <p className="text-xs text-slate-400">© 2026 YouKnowRecife. Plataforma independente de cidadania digital. Não substitui aconselhamento jurídico.</p>
        </div>
      </footer>
    </div>
  );
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 relative">
        ...
        <div className="w-full max-w-sm ykr-animate-up">
          ...
          <div className="mt-7 backdrop-blur-sm bg-white/60 ykr-card p-6 space-y-4" style={{ boxShadow: "0 20px 50px -20px rgba(15,23,42,.15)" }}>
            <button className="w-full ykr-btn-ghost py-2.5 flex items-center justify-center gap-2 text-sm">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {mode === "login" && <button className="text-xs font-medium text-[var(--yn-secondary)] hover:underline">Esqueci a senha</button>}
              </div>
              <div className="relative">
                <input className="ykr-input pr-9" type={showPw ? "text" : "password"} placeholder="••••••••" value={pw} onChange={(e) => setPw(e.target.value)} />
            ...
            <button onClick={() => onLogin(name || email.split("@")[0] || "Cidadão")} className="w-full ykr-btn-accent py-2.75 text-sm mt-2">
